...
if u'git' in nb_metadata:
    if u'suppress_outputs' in nb_metadata[u'git'] and nb_metadata[u'git'][u'suppress_outputs']:
...
ipy_version = int(json_in[u'nbformat'])-1 # nbformat is 1 more than actual version.

def strip_output_from_cell(cell):
    if u'outputs' in cell:
        cell[u'outputs'] = []
    if u'prompt_number' in cell:
        del cell[u'prompt_number']
    if u'execution_count' in cell:
        cell[u'execution_count']=None


if ipy_version == 2:
    for sheet in json_in[u'worksheets']:
        for cell in sheet[u'cells']:
            strip_output_from_cell(cell)
else:
    for cell in json_in[u'cells']:
        strip_output_from_cell(cell)
